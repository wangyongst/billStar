...
public class ValidateUtils {
	...
	public static boolean isIP(String ip) {
		String segment = "(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])";
		String reg = "^" + segment + "\\." + segment + "\\." + segment + "\\." + segment + "$";
	...
	public static List<String> generateIDCard(int count) {
		...
		if (count + a > 1000000000) {
			throw new BizException("NumberTooBig","数量过大");
		}
		List<String> list = new ArrayList<String>();
		char[] charArray = new char[] { '1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2' };
		int[] intArray = new int[] { 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2 };
		for (int b = 1; b <= count; b++) {
			...
			for (int i = 0; i < 17; i++) {
				si += (chars[i] - '0') * intArray[i];
			}
			char last = charArray[si % 11];
			String card = s + last;
	...
	public static boolean isEmptyString(String validateStr) {
		return null == validateStr || "".equals(validateStr.trim());
	}
	 ...
	 * @return boolean
	 */
	public static <T> boolean isEmptyCollection(Collection<T> validateColl) {
	 ...
	 * @return boolean
	 */
	public static <T> boolean isNotEmptyCollection(Collection<T> validateColl) {
	...
	public static <T, V> boolean isEmptyMap(Map<T, V> res) {
		return null == res || res.isEmpty();
	}

	public static <T, V> boolean isNotEmptyMap(Map<T, V> mapObj) {
		return !isEmptyMap(mapObj);
	}
	...
	public static boolean isEmptyObjectOrString(Object validateObj) {
		return null == validateObj || "".equals(validateObj.toString().trim());
	}
