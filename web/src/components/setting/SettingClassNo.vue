  ...
  <el-row id="body">
    ...
    <el-col :span="24">
      <el-table v-loading="loading" :data="list" class="bill-table">
        ...
        <el-table-column prop="name" label="班别"></el-table-column>
    ...
    <!--    新增修改框-->
    <el-dialog class="bill-dialog" v-bind:title="classNo.type === 0 ?'新增班别' : '修改班别'"
...
<script>
  export default {
    ...
    methods: {
      ...
      deleteItem(id) {
        const _this = this;
        _this.httpUtils.appPost('/sys/class/no/delete?id=' + id).then(function (res) {
          _this.listClassNo();
      ...
      updatePost() {
        const _this = this;
        _this.httpUtils.appPost('/sys/class/no/update', _this.classNo).then(function (res) {
          _this.listClassNo();
      ...
      createPost() {
        const _this = this;
        _this.httpUtils.appPost('/sys/class/no/save', _this.classNo).then(function (res) {
          _this.listClassNo();
          _this.createDialogVisible = false;
          _this.baseSuccessNotify(res);
        }, _this.operateFail);
